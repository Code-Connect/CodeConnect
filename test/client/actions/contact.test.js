...
describe('mentor test ', () => {
    ...
    //ich muss einen test schreiben welcher beide funktionen verbindet
    it('add Projects to CodeConnect', () => {
        console.log("add project locally to the platform, but without updating the database");
        // Mock den store einfach nur kopieren und einfügen
        // state.projects.repos
        const store = mockStore({
            projects: {
                repos: ["mockObject1", "mockObject2"]
            }
        });
        return store.dispatch(actions.addProjectsToCodeConnect("mockObject1")).then(() => {
